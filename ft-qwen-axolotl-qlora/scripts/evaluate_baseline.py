...
def parse_arguments():
    parser = argparse.ArgumentParser(description="基准模型LogiQA评估脚本")
    ...
    parser.add_argument("--data_path", type=str, default="../data/logiqa_test.jsonl",
                      help="LogiQA数据集路径")
    parser.add_argument("--output_path", type=str, default="../output/logiqa_results_baseline.json",
                      help="评估结果输出路径")
    parser.add_argument("--device", type=str, default="cuda",
                      help="设备: cpu, cuda, cuda:0, etc.")
...
def evaluate_baseline_model(args):
    ...
    logger.info(f"评估结果将保存至: {args.output_path}")
    ...
    logger.info(f"开始评估基准模型，共{len(logiqa_data)}个样本")
    ...
    # 加载基础模型和tokenizer
    logger.info(f"加载基准模型: {args.base_model}")
    ...
    logger.info(f"基准模型评估完成! 准确率: {accuracy:.4f} ({correct_count}/{len(logiqa_data)})")
    ...
    try:
        ...
        logger.info(f"基准模型评估结果已保存至: {args.output_path}")
        
        # 创建一个备份文件，确保能被找到
        backup_path = os.path.join(output_dir, "baseline_results.json")
        with open(backup_path, 'w', encoding='utf-8') as f:
            json.dump(result_data, f, ensure_ascii=False, indent=2)
        logger.info(f"创建了备份结果文件: {backup_path}")
    except Exception as e:
        logger.error(f"保存评估结果时出错: {e}")
        
        # 尝试使用替代路径保存
        try:
            alt_path = os.path.join(output_dir, "logiqa_results_baseline_alt.json")
            with open(alt_path, 'w', encoding='utf-8') as f:
                json.dump(result_data, f, ensure_ascii=False, indent=2)
            logger.info(f"使用替代路径保存评估结果: {alt_path}")
        except Exception as e2:
            logger.error(f"使用替代路径保存也失败: {e2}")
